...
public class FlexibleColorPicker : MonoBehaviour {
    ...
    */
    ...
    public struct Picker {
        ...
        [Tooltip("Marker for 2D picker (main)")]
        public RectTransform marker;
        [Tooltip("Marker for horizontal slider")]
        public RectTransform markerHor;
        [Tooltip("Marker for vertical slider")]
        public RectTransform markerVer;
        [Tooltip("If true, this picker is horizontal; if false, vertical. Set in Inspector.")]
    ...
    private float rainbowS = 1f;
    private float rainbowV = 1f;
    ...
    [Tooltip("Allow the hex input to accept 'RAINBOW' and animate hue over time.")]
    public bool allowRainbowInput = true;
    [Tooltip("Hue speed for RAINBOW mode (0-6 range per second).")]
    public float rainbowHueSpeed = 0.5f;
    ...
    */
    ...
    // picker: Picker構造体, v: marker位置, setX/setY: どちらの軸を使うか
    private void SetMarker(Picker picker, Vector2 v, bool setX, bool setY) {
        RectTransform marker = null;
        RectTransform offMarker = null;
        if(setX && setY)
            marker = picker.marker;
        else if(setX) {
            marker = picker.markerHor;
            offMarker = picker.markerVer;
        }
        else if(setY) {
            marker = picker.markerVer;
            offMarker = picker.markerHor;
        }
        if(offMarker != null)
        ...
        if(setX)
            anchoredPos.x = (v.x - parent.pivot.x) * parentSize.x;
        if(setY)
            anchoredPos.y = (v.y - parent.pivot.y) * parentSize.y;
    ...
    */
    ...
    private void UpdateDynamic(PickerType type) {
        ...
        if(m != p.dynamicMaterial)
            p.dynamicMaterial.CopyPropertiesFromMaterial(m);
    }
    ...
    */
    ...
    private void UpdateHex() {
        if(hexInput == null || !hexInput.gameObject.activeInHierarchy)
            return;
        if(rainbowMode) {
            hexInput.SetTextWithoutNotify("RAINBOW");
            return;
        }
    ...
    private void TypeHex(string input, bool finish) {
        ...
        if(!finish && allowRainbowInput) {
            if(trimmed.StartsWith("#", StringComparison.Ordinal) && trimmed.Length > 1) {
                char first = trimmed[1];
                if(first == 'R' || first == 'r') {
                    string rainbowInput = trimmed.Substring(1);
                    hexInput.SetTextWithoutNotify(rainbowInput);
                    hexInput.caretPosition = rainbowInput.Length;
                    return;
                }
            }

            if(trimmed.Length > 0 && (trimmed[0] == 'R' || trimmed[0] == 'r')) {
                string normalizedRainbow = trimmed.StartsWith("#") ? trimmed.Substring(1) : trimmed;
                if(string.Equals(normalizedRainbow, "RAINBOW", StringComparison.OrdinalIgnoreCase)) {
                    EnterRainbowMode();
                }
                return;
            }
        ...
        if(allowRainbowInput && string.Equals(normalized, "RAINBOW", StringComparison.OrdinalIgnoreCase)) {
            EnterRainbowMode();
            return;
        }
        ...
        if(finish) {
            if(string.IsNullOrEmpty(input) || !IsValidHexInput(trimmed))
                input = "#FFFFFF";
        }
    ...
    */
    ...
    private static bool IsValidHexInput(string input) {
        if(string.IsNullOrEmpty(input))
            return false;
        ...
        for(int i = 0; i < normalized.Length; i++) {
            char c = char.ToUpper(normalized[i]);
            if(!IsValidHexChar(c))
                return false;
        }
    ...
    private void UpdateRainbowColor() {
        if(rainbowHueSpeed <= 0f)
            return;

        rainbowHue = Mathf.Repeat(rainbowHue + (Time.unscaledDeltaTime * rainbowHueSpeed), HUE_LOOP);
        Color newColor = HSVToRGB(rainbowHue, rainbowS, rainbowV);
